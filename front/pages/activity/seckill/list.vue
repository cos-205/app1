      ...
      >
        <view class="goods-box ss-m-b-20" v-for="item in state.pagination.data" :key="item.id">
          <s-goods-column
            ...
            :seckillTag="true"
            @click="
              xxep.$router.go('/pages/goods/seckill', {
                id: item.id,
...
<script setup>
  ...
  const headerBg = xxep.$url.css('/assets/addons/cus/uniapp/goods/seckill-header.png');
  ...
  const endTime = computed(() => {
    if (useDurationTime(state.activityInfo.start_time).ms > 0) {
      return {
        ...useDurationTime(state.activityInfo.start_time),
        text: '距开始',
      };
    } else {
      if (state.activityInfo.end_time) {
        return {
          ...useDurationTime(state.activityInfo.end_time),
          text: '距结束',
        };
      }
    }
