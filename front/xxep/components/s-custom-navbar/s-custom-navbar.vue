...
<script setup>
  ...
  // 单元格大小
  const windowWidth = xxep.$platform.device.windowWidth;
  const cell = computed(() => {
    if (unref(navList).length) {
      let cell = (windowWidth - 90) / 8;
      // #ifdef MP
      cell = (windowWidth - 80 - unref(xxep.$platform.capsule).width) / 6;
      // #endif
      return cell;
    }
  });
      ...
      return {
        background:
          props.data.type == 'color'
            ? props.data.color
            : `url(${xxep.$url.cdn(props.data.src)}) no-repeat top center / 100% 100%`,
      };
    }
  });
