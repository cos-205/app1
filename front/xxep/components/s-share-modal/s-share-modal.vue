<template>
  ...
  <view>
    ...
    <view v-if="state.showShareGuide" class="guide-wrap">
      <image
        class="guide-image"
        :src="xxep.$url.static('/assets/addons/cus/uniapp/share/share_guide.png')"
      ></image>
    </view>
      ...
      <view class="share-box">
        <view class="share-list-box ss-flex">
          ...
          >
            <image
              class="share-img"
              :src="xxep.$url.static('/assets/addons/cus/uniapp/share/share_wx.png')"
              mode=""
            ></image>
          ...
          >
            <image
              class="share-img"
              :src="xxep.$url.static('/assets/addons/cus/uniapp/share/share_poster.png')"
              mode=""
            ></image>
          ...
          >
            <image
              class="share-img"
              :src="xxep.$url.static('/assets/addons/cus/uniapp/share/share_link.png')"
              mode=""
            ></image>
...
<script setup>
  ...
  import { showShareModal, closeShareModal, showAuthModal } from '@/xxep/hooks/useModal';

  const show = computed(() => xxep.$store('modal').share);
  const shareConfig = computed(() => xxep.$store('app').platform.share);
  ...
  const onShareByForward = () => {
    ...
    // #ifdef H5
    if (['WechatOfficialAccount', 'H5'].includes(xxep.$platform.name)) {
