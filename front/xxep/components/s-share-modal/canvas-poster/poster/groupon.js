...
const groupon = (poster) => {
  ...
  return {
    background: formatImageUrlProtocol(xxep.$url.cdn(xxep.$store('app').platform.share.posterInfo.groupon_bg)),
    list: [
      {
        name: 'nickname',
        type: 'text',
      ...
      },
      {
        name: 'avatar',
        type: 'image',
        val: formatImageUrlProtocol(xxep.$url.cdn(userInfo.avatar)),
        x: width * 0.04,
        y: width * 0.04,
        width: width * 0.14,
        height: width * 0.14,
      ...
      // #ifdef MP-WEIXIN
      {
        name: 'wxacode',
        type: 'image',
        val: xxep.$api.third.wechat.getWxacode(poster.shareInfo.path),
        x: width * 0.75,
        y: width * 1.3,
        width: width * 0.2,
        height: width * 0.2,
