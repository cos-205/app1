...
<template>
  ...
  >
    ...
    <view v-if="data.more.show" class="more-box ss-flex ss-col-center" @tap="xxep.$router.go(data.more.url)">
...
<script setup>
  ...
  // 组件样式
  const elStyles = {
    background: `url(${xxep.$url.cdn(props.data.src)}) no-repeat top center / 100% 100%`,
