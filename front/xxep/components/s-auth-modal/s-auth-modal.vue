  ...
  <su-popup :show="authType !== ''" round="10" :showClose="true" @close="closeAuthModal">
    <view class="login-wrap">
      ...
      >
        ...
        <view v-if="xxep.$platform.name === 'WechatMiniProgram'" class="ss-flex register-box">
        ...
        <button
          v-if="xxep.$platform.name !== 'WechatMiniProgram'"
...
<script setup>
  ...
  import { closeAuthModal, showAuthModal } from '@/xxep/hooks/useModal';
  ...
  const thirdLogin = async (provider) => {
    if (!state.protocol) {
      ...
      xxep.$helper.toast('请勾选同意');
      return;
    }
    const loginRes = await xxep.$platform.useProvider(provider).login();
    if (loginRes) {
      closeAuthModal();
