...
<script setup>
  ...
  import { showAuthModal, closeAuthModal } from '@/xxep/hooks/useModal';
  ...
  async function onConfirm() {
    ...
    if (!nickname) {
      xxep.$helper.toast('请输入昵称');
      return;
    }
    if (!avatar) {
      xxep.$helper.toast('请选择头像');
      return;
    }
    const { code, msg } = await xxep.$api.user.updateMpUserInfo(model);
    if (code === 1) {
      xxep.$helper.toast('授权成功');
