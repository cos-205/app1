...
<template>
  <view>
    ...
    <button
      v-if="'WechatMiniProgram' === xxep.$platform.name"
...
<script setup>
  ...
  import { code, mobile } from '@/xxep/validate/form';
  import { showAuthModal, closeAuthModal, getSmsCode, getSmsTimer } from '@/xxep/hooks/useModal';
  ...
  async function changeMobileSubmit() {
    ...
    if (!validate) return;
    xxep.$api.user.changeMobile(state.model).then((res) => {
      if (res.code === 1) {
  ...
  async function getPhoneNumber(e) {
    if (e.detail.errMsg !== 'getPhoneNumber:ok') {
      ...
      let result = await xxep.$platform.useProvider().bindUserPhoneNumber(e.detail);
      if (result) {
        xxep.$store('user').getInfo();
        closeAuthModal();
      }
