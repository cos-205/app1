...
<script setup>
  ...
  const data = computed(() => xxep.$store('app').template.basic?.floatMenu);
  ...
  data.value?.list.forEach((i) => {
    if (data.value?.isText === 0) {
      state.content.push({ iconPath: xxep.$url.cdn(i.src), url: i.url });
    } else {
      state.content.push({ iconPath: xxep.$url.cdn(i.src), text: i.title.text + '', url: i.url });
    }
    state.pattern.push({ color: i.title.color });
  });
  function trigger(e) {
    xxep.$router.go(e.item.url);
  }
  function onTap() {
    if (state.show) {
      state.show = false;
