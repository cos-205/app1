  ...
  >
    <view class="page-main" :style="[bgMain]">
      ...
      <view class="page-body" :style="[bgBody]">
        ...
        <view
          v-if="navbar === 'inner'"
          :style="[{ paddingTop: xxep.$platform.navbar + 'px' }]"
        ></view>
...
<script setup>
  ...
  const sysStore = xxep.$store('sys');
  const userStore = xxep.$store('user');
  const appStore = xxep.$store('app');
  ...
  const bgMain = computed(() => {
    if (navbarMode.value === 'inner') {
      return {
        background: `${props.bgStyle.color} url(${xxep.$url.cdn(
          props.bgStyle.src,
  ...
  const shareInfo = computed(() => {
    if (props.onShareAppMessage === true) {
      ...
      if (!isEmpty(props.onShareAppMessage)) {
        xxep.$platform.share.updateShareInfo(props.onShareAppMessage);
  ...
  onShow(() => {
    if (!isEmpty(shareInfo.value)) {
      xxep.$platform.share.updateShareInfo(shareInfo.value);
    }
  });
</script>
