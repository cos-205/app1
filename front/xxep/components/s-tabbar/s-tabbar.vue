  ...
  <view class="u-page__item" v-if="tabbar?.list.length > 0">
    ...
    >
      <su-tabbar-item
        ...
        :centerImage="xxep.$url.cdn(item.inactiveIcon)"
        @tap="xxep.$router.go(item.url)"
      >
        <template v-slot:active-icon>
          <image
            v-if="item.layout !== 2"
            class="u-page__item__slot-icon"
            :src="xxep.$url.cdn(item.activeIcon)"
          ></image>
        </template>
        <template v-slot:inactive-icon>
          <image
            v-if="item.layout !== 2"
            class="u-page__item__slot-icon"
            :src="xxep.$url.cdn(item.inactiveIcon)"
          ></image>
        </template>
      </su-tabbar-item>
    </su-tabbar>
...
<script setup>
  ...
  const tabbar = computed(() => {
    return xxep.$store('app').template.basic?.tabbar;
  });
