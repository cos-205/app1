...
import third from '@/xxep/api/third';
...
export default {
  ...
  async init(callback) {
    ...
    const { code, data } = await third.wechat.jssdk({
      ...
      jweixin.checkJsApi({
        ...
        success: function (res) {
          if (!res.checkResult['requestMerchantTransfer']) {
            $helper.toast('你的微信版本过低，请更新至最新版本');
          } else {
            window.WeixinJSBridge.invoke('requestMerchantTransfer', { ...data }, function (res) {
              console.log('requestMerchantTransfer res:', res);
