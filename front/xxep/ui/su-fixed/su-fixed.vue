  ...
  <view class="ui-fixed">
      ...
      :style="[
        {
          ...
          bottom: insetHeight,
          zIndex: index + xxep.$zIndex.navbar,
        },
    ...
    >
      <view
        ...
        @tap="toTop"
        :style="[{ zIndex: index + xxep.$zIndex.navbar }]"
      >
        <slot></slot>
        <view
      ...
      <view
        ...
        :class="[ui, bg]"
        :style="[
          { zIndex: index + xxep.$zIndex.navbar - 1 },
          bgStyles,
...
<script setup>
  ...
  onPageScroll((e) => {
    let top = e.scrollTop;
    state.scrollTop = top;
    state.opacityVal = top > xxep.$platform.navbar ? 1 : top * 0.01;
  });
    ...
    if (unref(props.bottom)) {
      state.fixed =
        ...
        state.content.bottom -
          xxep.$platform.device.windowHeight +
      ...
      state.fixed =
        value >=
        state.content.top -
          (unref(props.noNav) ? unref(props.val) : unref(props.val) + xxep.$platform.navbar);
    }
  };
