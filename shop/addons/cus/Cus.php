...
use addons\cus\library\Hook;

/**
 * Cus插件 v3.0.0
 */
class Cus extends Addons
{
    ...
    public function appInit()
    {
        ...
        if (request()->isCli()) {
            \think\Console::addDefaultCommands([
                'addons\cus\console\CusChat',
                'addons\cus\console\CusHelp'
            ]);
        }
    ...
    {
        $newMenu = [];
        $config_file = ADDON_PATH . "cus" . DS . 'config' . DS . "menu.php";
        if (is_file($config_file)) {
            $newMenu = include $config_file;
        }
        $oldMenu = AuthRule::where('name', 'like', "cus%")->select();
