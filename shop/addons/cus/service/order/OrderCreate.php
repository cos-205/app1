...
namespace addons\cus\service\order;

use think\Db;
use addons\cus\service\goods\GoodsService;
...
use app\admin\model\cus\order\Address as OrderAddress;
use app\admin\model\cus\order\Invoice as OrderInvoice;
use app\admin\model\cus\dispatch\Dispatch;
use app\admin\model\cus\user\Coupon;
use app\admin\model\cus\user\Address as UserAddress;
use app\admin\model\cus\user\Invoice as UserInvoice;
...
class OrderCreate
{
    ...
    private function equalDispatch($dispatch_amount, $currentBuyInfos, $data)
    {
        ...
        $current_num = 0;
        $equal_dispatch_amount = '0';
        foreach ($this->goodsList as $key => &$buyInfo) {
            if (in_array($buyInfo['goods_id'], $goodsIds)) {
                $current_num++;
                if ($current_num == count($goodsIds)) {
                    // 本次分配的最后一个商品
                    $current_dispatch_amount = round(bcsub($dispatch_amount, $equal_dispatch_amount, 2), 2);
                } else {
                    $scale = 0;                             // 重量或者数量计算比例
                    if ($finalExpress['type'] == 'number') {
                        // 按件
                ...
                $equal_dispatch_amount = bcadd($equal_dispatch_amount, $current_dispatch_amount, 2);        // 记录已分配的运费金额
    ...
    private function createOrder($result)
    {
        $order = Db::transaction(function () use ($result) {
            ...
            $ext['invoice_amount_type'] = $result['invoice_amount_type'];
            $ext['invoice_amount'] = $result['invoice_amount'];
            $orderData['ext'] = $ext;
