...
namespace addons\cus\service\commission;

use addons\cus\service\commission\Config;
use app\admin\model\cus\user\User as UserModel;
use app\admin\model\cus\commission\Agent as AgentModel;
use app\admin\model\cus\commission\Level as LevelModel;
...
class Agent
{
    ...
    public function createNewAgent($event = '', $applyInfo = [])
    {
        ...
        switch ($condition['type']) {
            case 'apply':   // 直接自助申请
                ...
                break;
            case 'goods':   // 需购买指定产品
                $isBuy = \app\admin\model\cus\order\Order::hasWhere('items', function ($query) use ($condition) {
    ...
    public function createAsyncAgentUpgrade($user_id = 0)
    {
        ...
        \think\Queue::push('\addons\cus\job\Commission@agentUpgrade', [
            'user_id' => $user_id
        ], 'cus');
    }
