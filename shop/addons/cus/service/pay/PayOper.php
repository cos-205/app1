...
namespace addons\cus\service\pay;
...
class PayOper
{
        ...
        if ($order->status == $order::STATUS_PAID) {
            ...
            if ($order_type == 'order') {
                if ($pay_mode == 'offline') {
                    ...
                    // 支付成功后续使用异步队列处理
                    \think\Queue::push('\addons\cus\job\OrderPaid@paid', ['order' => $order, 'user' => $user], 'cus-high');
                }
            } else if ($order_type == 'trade_order') {
                // 支付成功后续使用异步队列处理
                \think\Queue::push('\addons\cus\job\trade\OrderPaid@paid', ['order' => $order, 'user' => $user], 'cus-high');
            }
            ...
            if ($order_type == 'order') {
                ...
                // 支付成功后续使用异步队列处理
                \think\Queue::push('\addons\cus\job\OrderPaid@offline', ['order' => $order, 'user' => $user], 'cus-high');
            }
        }
    ...
    public function getOrderModel($order_type)
    {
        switch ($order_type) {
            case 'trade_order':
                $orderModel = '\\app\\admin\\model\\cus\\trade\\Order';
                break;
            case 'order':
                $orderModel = '\\app\\admin\\model\\cus\\order\\Order';
                break;
            default:
                $orderModel = '\\app\\admin\\model\\cus\\order\\Order';
                break;
