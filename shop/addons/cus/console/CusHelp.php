...
class CusHelp extends Command
{
    ...
    protected $commands = [
        ...
        ['code' => "1", 'name' => 'all', 'desc' => 'cus 帮助工具列表'],
    ...
    {
        $this->setName('cus:help')
            ->addArgument('code', Argument::OPTIONAL, "请输入操作编号")
            ->setDescription('cus 帮助命令');
    }
    ...
    {
        @unlink(RUNTIME_PATH . 'storage/queue/cus.log');
        @unlink(RUNTIME_PATH . 'storage/queue/cus-high.log');
        ...
        // 添加验证队列
        Queue::push('\addons\cus\job\Test@cus', [], 'cus');       // 普通队列
        Queue::push('\addons\cus\job\Test@cusHigh', [], 'cus-high');     // 高优先级队列

        $this->output->writeln('测试队列添加成功');
        $this->output->writeln('请检查 ' . str_replace('\\', '/', RUNTIME_PATH . 'storage/queue') . '目录下是否存在如下文件，并且文件内容为当前测试的时间');
        $this->output->newLine();
        $this->output->writeln(str_replace('\\', '/', RUNTIME_PATH . 'storage/queue') . 'cus.log               // 如果没有该文件，则普通优先级队列未监听');
        $this->output->writeln(str_replace('\\', '/', RUNTIME_PATH . 'storage/queue') . 'cus-high.log          // 如果没有该文件，则高优先级队列未监听');
        $this->output->newLine();
