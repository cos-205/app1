...
use app\admin\model\cus\decorate\Decorate;
use app\admin\model\cus\decorate\Page;
...
class Index extends Common
{
    ...
    public function pageSync()
    {
        ...
        foreach ($pages as $page) {
            if (!empty($page['meta']['sync']) && $page['meta']['sync']) {
                $data = \app\admin\model\cus\data\Page::getByPath($page['path']);
                ...
                if ($data) {
                    ...
                    \app\admin\model\cus\data\Page::create([
                        'name' => $name,
                        'group' => $group,
                        'path' => $page['path']
                    ]);
    ...
    public function page()
    {
        ...
        $template = \app\admin\model\cus\decorate\Decorate::typeDiypage()->with('diypage')->where('id', $id)->find();
        if (!$template) {
    ...
    public function feedback()
    {
        ...
        $result = \app\admin\model\cus\Feedback::create($params);
        if ($result) {
            $this->success('感谢您的反馈');
        }
    }
