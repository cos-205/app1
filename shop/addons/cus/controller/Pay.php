...
use app\admin\model\cus\trade\Order as TradeOrderModel;
use think\Log;
use think\Db;
use addons\cus\service\pay\PayRefund;
use Yansongda\Pay\Pay as YansongdaPay;

class Pay extends Common
{

    protected $noNeedLogin = ['alipay', 'notify', 'notifyRefund', 'notifyTransfer'];
        ...
        if ($payment == 'wechat') {
            if (in_array($platform, ['WechatOfficialAccount', 'WechatMiniProgram'])) {
                if (isset($openid) && $openid) {
                    ...
                    $oauth = \app\admin\model\cus\ThirdOauth::where([
    ...
    {
        // TODO 需校验 SUCCESS、CANCELLED和FAIL三种情况
        Log::write('pay-notify-transfer-comein:');
        ...
        $result = $payService->notifyTransfer(function ($action, $withdrawSn, $originData) use ($payment) {
            Db::transaction(function () use ($action, $withdrawSn, $originData) {
                // 交给提现服务处理
                $widthdrawService = new WithdrawService();
                $widthdrawService->handleWechatTransferNotify(
                    $action,
                    $withdrawSn,
                    $originData,
                );
