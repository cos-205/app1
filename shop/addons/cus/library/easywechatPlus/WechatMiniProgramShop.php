...
namespace addons\cus\library\easywechatPlus;

use addons\cus\exception\CusException;
use addons\cus\service\order\shippingInfo\OrderShippingInfo;
use addons\cus\service\order\shippingInfo\TradeOrderShippingInfo;
use app\admin\model\cus\data\WechatExpress;
...
class WechatMiniProgramShop extends EasywechatPlus
{
    ...
    public function uploadShippingInfos($order, $express = null, $type = 'send')
    {
        try {
            ...
            foreach ($uploadParams as $key => $params) {
                ...
                if ($result['errcode'] != 0) {
                    throw new CusException('获取失败: errcode:' . $result['errcode'] . '; errmsg:' . $result['errmsg']);
                }
            }
        } catch (\Exception $e) {
    ...
    private function uploadShippingInfo($params)
    {
        ...
        $result = \addons\cus\facade\HttpClient::request('post', $add_template_url, [
    ...
    public function isTradeManaged($is_force = false)
    {
        try {
            ...
            if (!$mini_appid) {
                // 没有配置微信小程序参数
                throw new CusException('微信小程序发货管理查询失败，没有配置微信小程序');
            }
            ...
            if ($result['errcode'] != 0 && $result['errcode'] != '48001') {
                ...
                throw new CusException('查询是否有微信发货信息管理权限失败: errcode:' . $result['errcode'] . '; errmsg:' . $result['errmsg']);
            }
            ...
            format_log_error($e, 'isTradeManaged', '查询微信发货信息管理权限失败');
            return 0;
        }
    }
    ...
    public function setMessageJumpPath($is_force = false, $path = 'pages/order/detail?comein_type=wechat')
    {
        ...
        if ($result['errcode'] != 0) {
            throw new CusException('设置微信发货消息跳转地址失败: errcode:' . $result['errcode'] . '; errmsg:' . $result['errmsg']);
        }
    ...
    public function getDelivery($is_force = false)
    {
        ...
        $result = \addons\cus\facade\HttpClient::request('post', $get_delivery_url, [
            'body' => '{}',
            'query' => ["access_token" => $access_token['access_token']],
            'headers' => ['Content-Type' => 'application/json']
        ]);
        ...
        if ($result['errcode'] != 0) {
            throw new CusException('获取微信 delivery 列表失败: errcode:' . $result['errcode'] . '; errmsg:' . $result['errmsg']);
        }
