...
namespace addons\cus\library\activity\traits;

use addons\cus\facade\Redis;
use addons\cus\facade\ActivityRedis;
use app\admin\model\cus\activity\Activity;
use app\admin\model\cus\activity\Groupon as ActivityGroupon;
use app\admin\model\cus\activity\GrouponLog;
use app\admin\model\cus\order\Order;
use app\admin\model\cus\order\OrderItem;
use app\admin\model\cus\data\FakeUser;
...
trait Groupon
{
    ...
    protected function joinNewGroupon($order, $user, $item, \Closure $grouponCb = null)
    {
        ...
        if ($expire_time > 0) {
            ...
            \think\Queue::later($expire_time, '\addons\cus\job\GrouponAutoOper@expire', [
                'activity' => $activity,
        ...
        if ($fictitious_time > 0) {
            ...
            \think\Queue::later($fictitious_time, '\addons\cus\job\GrouponAutoOper@fictitious', [
                'activity' => $activity,
                'activity_groupon_id' => $activityGroupon->id
            ], 'cus');
        }
