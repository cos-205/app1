...
namespace addons\cus\library\activity\provider;

use addons\cus\library\activity\traits\Groupon as GrouponTrait;
...
class GrouponLadder extends Base
{
    ...
    public function buyCheck($buyInfo, $activity)
    {
        ...
        if (!$groupon_id && (!$currentLadder || $currentLadder['ladder'] <= 1)) {
            throw new CusException('请选择正确的开团阶梯');
        }
        ...
        if ($buy_type == 'alone') {
            // 不允许单独购买
            if (!$is_alone) {
                throw new CusException('该商品不允许单独购买');
            }
        } else {
            // 拼团，临时将拼团价设置为商品价格
        ...
        if ($currentSkuPrice['stock'] < $need_num) {
            if ($need_add_num && $is_alone && !$groupon_id && $buy_type == 'groupon') {
                throw new CusException('商品库存不足以开团，请选择单独购买');
            } else if ($buy_type == 'alone') {
                throw new CusException('商品库存不足');
            } else {
                throw new CusException('商品库存不足以开团');
            }
        }
