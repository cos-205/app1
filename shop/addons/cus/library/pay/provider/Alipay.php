...
class Alipay extends Base
{
    ...
    public function pay($order, $config = [], $from = null) 
    {
        ...
        if (in_array($this->platform, ['WechatOfficialAccount', 'WechatMiniProgram', 'H5'])) {
            ...
            if (!$from == 'url') {
                ...
                return request()->domain() . '/addons/cus/pay/alipay?pay_sn=' . $order['out_trade_no'] . '&platform=' . $this->platform . '&return_url=' . urlencode($this->config['alipay']['default']['return_url']);
            }
        } 
    ...
    {
        $config['notify_url'] = request()->domain() . '/addons/cus/pay/notify/payment/alipay/platform/' . $this->platform;
