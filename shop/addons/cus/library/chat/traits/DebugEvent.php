...
namespace addons\cus\library\chat\traits;
...
trait DebugEvent
{
    ...
    public function register($event, \Closure $cb)
    {
        $this->socket->on($event, function ($data, $callback = null) use ($cb) {
            try {
                ...
                // 将错误报告给前端
                $this->sender->errorSocket('custom_error', ($this->io->debug || $e instanceof CusException) ? $e->getMessage() : 'socket 服务器异常');
            }
        });
    }
    ...
    public function exec($httpConnection, \Closure $callback) 
    {
        try {
            ...
            // 将错误报告给前端
            $httpConnection->send(($this->io->debug || $e instanceof CusException) ? $e->getMessage() : 'socket 服务器异常');
        }
    }
