...
if (!function_exists('sheep_config')) {
    ...
    {
        return \app\admin\model\cus\Config::getConfigs($code, $cache);
    }
}
...
if (!function_exists('auth_admin')) {
    function auth_admin()
    {
        if (\app\admin\library\Auth::instance()->isLogin()) {
            ...
            if ($admin) {
                return \app\admin\model\cus\Admin::where('id', $admin['id'])->find();
            }
        }
...
if (!function_exists('redis_cache')) {
    ...
    {
        $cache = new \addons\cus\library\RedisCache;
...
if (!function_exists('check_env')) {
    function check_env($need = [], $is_throw = true)
    {
        ...
        if (in_array('commission', $need)) {
            if (!class_exists(\addons\cus\listener\Commission::class)) {
                if ($is_throw) {
                    error_stop('请先升级 【cus】');
                } else {
                    return false;
