...
use addons\cus\library\Pipeline;
use addons\cus\service\order\OrderThrough;
use app\admin\model\cus\user\User;
use app\admin\model\cus\Admin;
use app\admin\model\cus\Cart;
use app\admin\model\cus\user\Coupon;
use app\admin\model\cus\order\Order as OrderModel;
use app\admin\model\cus\order\OrderItem;
use app\admin\model\cus\order\Invoice as OrderInvoiceModel;
use app\admin\model\cus\order\Express as OrderExpressModel;
use app\admin\model\cus\order\Action;
use app\admin\model\cus\order\Invoice;
use app\admin\model\cus\Config;
use addons\cus\service\StockSale;
use addons\cus\traits\CouponSend;
use addons\cus\service\order\OrderRefund;
use addons\cus\service\pay\PayOper;
use addons\cus\service\order\OrderOper;
use addons\cus\service\user\User as UserService;
use addons\cus\service\order\OrderDispatch;
use addons\cus\library\activity\traits\GiveGift;
use addons\cus\facade\Activity as ActivityFacade;
use addons\cus\facade\Wechat;
use addons\cus\library\easywechatPlus\WechatMiniProgramShop;
...
class Order
{
    ...
    private function newOrderNotify($order, $user)
    {
        ...
        $admins = collection(Admin::select())->filter(function ($admin) {
            return $admin->hasAccess($admin, [      // 订单列表或者订单详情权限
                'cus/order/order/index',
                'cus/order/order/detail'
            ]);
        });
        if (!$admins->isEmpty()) {
            Notify::send(
    ...
    public function orderApplyRefundAfter($params)
    {
        ...
        $admins = collection(Admin::select())->filter(function ($admin) {
            return $admin->hasAccess($admin, [      // 订单详情权限,订单拒绝全额退款，全额退款
                'cus/order/order/detail',
                'cus/order/order/applyrefundrefuse',
                'cus/order/order/fullrefund'
            ]);
        });
        if (!$admins->isEmpty()) {
            Notify::send(
    ...
    private function dispatchAfter($params, $type)
    {
        ...
        if (!$order->isOffline($order)) {        // 线下付款，未付款的不添加自动收货队列
            ...
            if ($confirm_days) {
                // 小于等于0， 不自动确认收货
                \think\Queue::later(($confirm_days * 86400), '\addons\cus\job\OrderAutoOper@autoConfirm', $params, 'cus');
            }
        }
    ...
    public function orderConfirmAfter($params)
    {
        ...
        if ($comment_days) {
            // 小于等于0， 不自动评价
            \think\Queue::later(($comment_days * 86400), '\addons\cus\job\OrderAutoOper@autoComment', $params, 'cus');
        }
    ...
    public function orderRefundAfter($params)
    {
        ...
        $refund_money = array_sum(array_column($items, 'refund_fee'));
        ...
        if ($refund_money > 0) {
            // 扣除退款金额的累计消费金额
            UserService::consume($user, -$refund_money);
        }
    ...
    private function autoCloseQueue($order, $activity, &$order_ext)
    {
        ...
        if ($close_minue) {
            ...
            \think\Queue::later(($close_minue * 60), '\addons\cus\job\OrderAutoOper@autoClose', ['order' => $order], 'cus');
        } else {
