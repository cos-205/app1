...
use addons\cus\service\order\OrderDispatch as OrderDispatchService;
...
class Order extends Common
{
    ...
    public function index()
    {
        if (!$this->request->isAjax()) {
            if (sheep_config('shop.platform.WechatMiniProgram.status')) {       // 如果开启了小程序平台
                ...
                $uploadshoppingInfo = new \addons\cus\library\easywechatPlus\WechatMiniProgramShop(Wechat::miniProgram());
                $uploadshoppingInfo->checkAndSetMessageJumpPath();
            }

            $exportConfig = (new \addons\cus\library\Export())->getConfig();
    ...
    public function getType()
    {
        ...
        $classify = (new \app\admin\model\cus\activity\Activity)->classifies();
    ...
    public function updateExpress($order_express_id = 0)
    {
        ...
        $orderAddress = OrderAddressModel::where('order_id', $orderExpress->order_id)->find();
        if (!$orderAddress) {
            $this->error('订单不存在');
        }
    ...
    public function action($id)
    {
        ...
        $morphs = [
            'user' => \app\admin\model\cus\user\User::class,
    ...
    public function export()
    {
        ...
        $export = new \addons\cus\library\Export();
        $params = [
            'file_name' => '订单列表',
