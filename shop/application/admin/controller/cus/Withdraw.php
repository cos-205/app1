...
use addons\cus\service\Withdraw as WithdrawLibrary;
...
class Withdraw extends Common
{
    ...
    public function handle($id)
    {
        ...
        if (!in_array($action, ['agree', 'withdraw', 'agree&withdraw', 'refuse', 'refund_commission', 'cancel'])) {
            $this->error('您的操作有误');
        }
    ...
    public function checkWechatTransferResult($id)
    {
        ...
        $withdrawLib = new WithdrawLibrary($withdraw->user_id);
        try {
            $withdrawLib->checkWechatTransferResult($withdraw);
        } catch (\Exception $e) {
            $this->error($e->getMessage());
        }
