...
namespace app\admin\model\cus\order\traits;

use app\admin\model\cus\order\Order;
use app\admin\model\cus\order\OrderItem;
use app\admin\model\cus\activity\Groupon;
...
trait OrderStatus
{
    ...
    {
        $btn_name = strpos(request()->url(),  'addons/cus') !== false ? 'btns' : 'backend_btns';
    ...
    public function setOrderItemStatusByOrder($order)
    {
        ...
        foreach ($order['items'] as $key => &$item) {
            if ((!in_array($order['status'], [Order::STATUS_PAID, Order::STATUS_COMPLETED]) && !$this->isOffline($order))       // 没有支付，并且也不是货到付款
                ...
                if (strpos(request()->url(), 'addons/cus') !== false) {
                    // 前端
