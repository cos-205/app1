...
-- 1. 修改流程记录表，添加缺失字段
-- 注意：使用 flow_status 而不是 status（避免与表状态字段冲突）
ALTER TABLE `fa_fuka_card_flow_log` 
  ADD COLUMN `flow_status` TINYINT(1) DEFAULT 1 COMMENT '流程状态:0=锁定,1=未支付,2=已支付待审核,3=已完成' AFTER `step_name`;

-- 检查是否已有 order_id 字段
ALTER TABLE `fa_fuka_card_flow_log` 
  ADD COLUMN `order_id` INT(11) UNSIGNED DEFAULT 0 COMMENT '关联订单ID' AFTER `card_id`;

-- 检查是否已有 extra_data 字段
ALTER TABLE `fa_fuka_card_flow_log` 
  ADD COLUMN `extra_data` TEXT COMMENT '额外数据(JSON格式)' AFTER `flow_status`;

-- 检查是否已有 auditor_id 字段
ALTER TABLE `fa_fuka_card_flow_log` 
  ADD COLUMN `auditor_id` INT(11) UNSIGNED DEFAULT 0 COMMENT '审核人ID' AFTER `complete_time`;

-- 检查是否已有 audit_time 字段
ALTER TABLE `fa_fuka_card_flow_log` 
...
-- 添加索引（如果不存在）
ALTER TABLE `fa_fuka_card_flow_log` 
  ADD INDEX `flow_status` (`flow_status`);
...
SELECT '====================================================================' AS '';
SELECT '已完成以下操作：' AS '';
SELECT '1. 修改流程记录表，添加flow_status等字段' AS '';
SELECT '2. 创建支付订单表' AS '';
SELECT '3. 创建协议流程配置表' AS '';
SELECT '4. 创建用户协议流程记录表' AS '';
SELECT '5. 更新流程配置数据（9个步骤）' AS '';
SELECT '6. 插入协议流程配置数据（5个阶段）' AS '';
SELECT '====================================================================' AS '';
