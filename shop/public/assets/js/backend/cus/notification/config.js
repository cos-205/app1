            ...
            const index = {
                setup() {
                    ...
                    function onEdit(event, channel) {
                        Fast.api.open(`cus/notification/config/edit?event=${event}&channel=${channel}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onSetStatus(status, event, channel) {
                        Fast.api.ajax({
                            url: `cus/notification/config/setStatus/id/${event}`,
                            type: 'POST',
                            data: {
                    ...
                    function getQrcode() {
                        Fast.api.ajax({
                            url: 'cus/wechat/admin/getQrcode?event=bind',
                            type: 'GET',
                            loading: false,
                        }, function (ret, res) {
                            if (res.code === 1) {
                    ...
                    function checkScanResult(eventId) {
                        ...
                        Fast.api.ajax({
                            url: 'cus/wechat/admin/checkScan?event=bind&eventId=' + eventId,
                            type: 'GET',
                            loading: false,
                        }, function (data, res) {
                            if (res.code === 1) {
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/notification/config/detail`,
                            type: 'GET',
                            data: {
                                event: state.event,
                    ...
                    function getTemplateId(is_delete) {
                        ...
                        Fast.api.ajax({
                            url: `cus/notification/config/getTemplateId`,
                            type: 'GET',
                            data: {
                                event: state.event,
                                channel: state.channel,
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                ...
                                Fast.api.ajax({
                                    url: `cus/notification/config/edit`,
                                    type: 'POST',
                                    data: {
                                        event: state.event,
                                        channel: state.channel,
