        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function getType() {
                        Fast.api.ajax({
                            url: 'cus/order/aftersale/getType',
                            type: 'GET',
                        }, function (ret, res) {
                            type.data = res.data
                            for (key in res.data) {
                    ...
                    function onDetail(id) {
                        Fast.api.open(`cus/order/aftersale/detail/id/${id}?id=${id}`, "详情", {
                            callback() {
                                getData()
                            }
                        })
        ...
        detail: () => {
            ...
            const detail = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/order/aftersale/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            state.data = res.data;
                            setStepActive()
                    ...
                    function onRefund() {
                        Fast.api.open(`cus/order/aftersale/refund/id/${state.id}?id=${state.id}&suggest_refund_fee=${state.data.suggest_refund_fee}`, "售后退款", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onRefuse() {
                        Fast.api.open(`cus/order/aftersale/refuse/id/${state.id}?id=${state.id}`, "拒绝售后", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onCompleted() {
                        Fast.api.ajax({
                            url: `cus/order/aftersale/completed/id/${state.id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getDetail()
                        }, function (ret, res) { })
                    ...
                    function onAddLog() {
                        Fast.api.open(`cus/order/aftersale/addLog/id/${state.id}?id=${state.id}`, "回复买家", {
                            callback() {
                                getDetail()
                            }
                        })
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/aftersale/refund/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/aftersale/refuse/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/aftersale/addLog/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
