            ...
            const index = {
                setup() {
                    ...
                    function onBatchHandle(type) {
                        ...
                        switch (type) {
                            case 'dispatch':
                                Fast.api.open(`cus/order/order/batchDispatch?order_ids=${ids.join(',')}`, "批量发货", {
                                    callback() {
                                        getData()
                                    }
                                })
                    ...
                    function onAftersale(item) {
                        Fast.api.open(`cus/order/aftersale/detail/id/${item.ext?.aftersale_id}?id=${item.ext?.aftersale_id}`, "售后详情", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onExport(type) {
                        ...
                        if (Config.save_type == 'download') {
                            window.location.href = `${Config.moduleurl}/cus/order/order/${type}?page=${pagination.page}&list_rows=${pagination.list_rows}&search=${search.search}`;
                            exportLoading.value = false;
                        } else if (Config.save_type == 'save') {
                            Fast.api.ajax({
                                url: `cus/order/order/${type}`,
                                type: 'GET',
                                data: {
                                    page: pagination.page,
                                    list_rows: pagination.list_rows,
                    ...
                    function onChangeFee(item) {
                        Fast.api.open(`cus/order/order/changeFee/id/${item.id}?id=${item.id}&pay_fee=${item.pay_fee}`, "改价", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onApplyRefundRefuse(id, index) {
                        ...
                        Fast.api.ajax({
                            url: `cus/order/order/applyRefundRefuse/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData();
                        }, function (ret, res) { })
                    ...
                    function onFullRefund(item, index) {
                        refundPopover[index] = false;
                        Fast.api.open(`cus/order/order/fullRefund/id/${item.id}?id=${item.id}`, "同意用户申请退款", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDispatch(id) {
                        Fast.api.open(`cus/order/order/dispatch?id=${id}`, "立即发货", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onOfflineRefuse(id, index) {
                        ...
                        Fast.api.ajax({
                            url: `cus/order/order/offlineRefuse/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData();
                        }, function (ret, res) { })
                    ...
                    function onOfflineConfirm(id, index) {
                        confirmPopover[index] = false;
                        Fast.api.ajax({
                            url: `cus/order/order/offlineConfirm/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData();
                        }, function (ret, res) { })
                    ...
                    function onDetail(id) {
                        Fast.api.open(`cus/order/order/detail/id/${id}?id=${id}`, "详情", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onAction(id) {
                        Fast.api.open(`cus/order/order/action/id/${id}?id=${id}`, "日志", {
                            callback() {
                                getData()
                            }
                        })
        ...
        detail: () => {
            ...
            const detail = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/order/order/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                    ...
                    function onDispatch() {
                        Fast.api.open(`cus/order/order/dispatch?id=${state.id}`, "立即发货", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onFullRefund() {
                        Fast.api.open(`cus/order/order/fullRefund/id/${state.id}?id=${state.id}`, "全部退款", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onConfirmMemo() {
                        Fast.api.ajax({
                            url: `cus/order/order/editMemo/id/${state.id}`,
                            type: 'POST',
                            data: memo.form,
                        }, function (ret, res) {
                            memo.flag = false;
                    ...
                    function onConfirmAddress() {
                        ...
                        delete tempForm.pcd;
                        Fast.api.ajax({
                            url: `cus/order/order/editConsignee/id/${state.id}`,
                            type: 'POST',
                            data: tempForm,
                        }, function (ret, res) {
                            getDetail();
                    ...
                    function onInvoice(item) {
                        ...
                        Fast.api.open(`cus/order/invoice/confirm/id/${item.id}?item=${encodeURI(JSON.stringify(params))}`, "确认开具", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onUpdateExpress(order_express_id, type) {
                        Fast.api.ajax({
                            url: `cus/order/order/updateExpress/order_express_id/${order_express_id}`,
                            type: 'GET',
                            data: {
                                type,
                            }
                    ...
                    function onRefund(item) {
                        Fast.api.open(`cus/order/order/refund/id/${state.id}/item_id/${item.id}?id=${state.id}&item_id=${item.id}&suggest_refund_fee=${item.suggest_refund_fee}`, "主动退款", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onComment(item) {
                        Fast.api.open(`cus/goods/comment/index?order_id=${state.id}&order_item_id=${item.id}`, "查看评价", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/order/order/add' : `cus/order/order/edit/id/${state.id}`,
                                    type: 'POST',
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/order/fullRefund/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
                ...
                setup() {
                    ...
                    function onConfirm() {
                        ...
                        if (state.dispatch_type == 'express') {
                            ...
                            Fast.api.ajax({
                                url: 'cus/order/order/customDispatch',
                                type: 'POST',
                                data: {
                                    order_id: state.id,
                                    order_item_ids,
        ...
        action: () => {
            ...
            const action = {
                setup() {
                    ...
                    function getAction() {
                        Fast.api.ajax({
                            url: `cus/order/order/action/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            state.data = res.data
                            return false
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/order/changeFee/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
                ...
                setup() {
                                ...
                                $.ajax({
                                    ...
                                    success: function (res) {
                                        ...
                                        if (res.code == 1) {
                                            ...
                                            Fast.api.open(`cus/order/order/dispatchList?method=upload`, "导入发货单发货", {
                                                callback: function () {
                                                    Fast.api.close()
                                                }
                                            });
                    ...
                    function onDispatchList() {
                        Fast.api.ajax({
                            ...
                            localStorage.setItem("batch-dispatch", JSON.stringify(res.data));
                            Fast.api.open(`cus/order/order/dispatchList?method=api`, "批量发货", {
                                callback: function () {
                                    Fast.api.close()
                                }
                            });
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/order/order/refund/id/${state.id}/item_id/${state.item_id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
