            ...
            const index = {
                setup() {
                    ...
                    function onActivity(key, title) {
                        if (key == 'full_coupon') {
                            Fast.api.addtabs(`cus/coupon/index`, '优惠券')
                        } else if (key == 'score_shop') {
                            Fast.api.addtabs(`cus/app/score_shop/index`, '积分商城')
                        } else if (key == 'wechat_mplive') {
                            Fast.api.addtabs(`cus/app/mplive/index`, '小程序直播')
                        } else {
                            Fast.api.addtabs(`cus/activity/activity/index?type=${key}&title=${title}`, title)
                        }
                    }
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/activity/activity/add?type=add&activity_type=${state.type}`, "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/activity/activity/edit?type=edit&activity_type=${state.type}&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/activity/activity/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onRecyclebin() {
                        Fast.api.open(`cus/activity/activity/recyclebin?activity_type=${state.type}`, "回收站", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onGroupon(id) {
                        Fast.api.addtabs(`cus/activity/groupon/index?activity_id=${id}`, "拼团列表", {
                            callback() {
                                getData()
                            }
                        })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/activity/activity/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            ...
                            if (form.model.rules.limit_num > 0) {
                    ...
                    function onSelectCoupon(index) {
                        Fast.api.open(`cus/coupon/select?multiple=true&status=hidden`, "选择优惠券", {
                            callback(data) {
                    ...
                    function onSelectArea() {
                        ...
                        Fast.api.open(`cus/data/area/select?selected=${encodeURI(JSON.stringify(selected))}`, "选择地区", {
                            callback(data) {
                    ...
                    function onSelectGoods() {
                        ...
                        Fast.api.open(`cus/goods/goods/select?multiple=true&ids=${ids.join(',')}`, "选择商品", {
                            callback(data) {
                                data.forEach((item) => {
                    ...
                    function onSetActivitySkuPrices(index, id) {
                        ...
                        Fast.api.open(`cus/activity/activity/skus?activityStatus=${state.activityStatus}&goods_id=${id}`, "设置商品", {
                            callback(data) {
                    ...
                    function onSelectRichtext() {
                        Fast.api.open(`cus/data/richtext/select`, "选选择活动说明", {
                            callback(data) {
                    ...
                    function onConfirm() {
                        ...
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/activity/activity/add' : `cus/activity/activity/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: submitForm
                                }, function (ret, res) {
                                    Fast.api.close()
            ...
            const select = {
                setup() {
                    ...
                    function getData() {
                        ...
                        let search = composeFilter(tempSearch);
                        Fast.api.ajax({
                            url: 'cus/activity/activity/select',
                            type: 'GET',
                            data: {
                                type: state.type,
                                page: pagination.page,
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/activity/activity/recyclebin',
                            type: 'GET',
                            data: {
        ...
        skus: () => {
            ...
            const skus = {
                setup() {
                    ...
                    function getSkus() {
                        Fast.api.ajax({
                            url: 'cus/activity/activity/skus',
                            type: 'GET',
                            data: {
                                id: state.model.id,
