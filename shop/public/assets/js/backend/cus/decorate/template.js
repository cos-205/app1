        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/decorate/template/add?type=add&template=${state.filter.data.type}`, "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/decorate/template/edit?type=edit&id=${id}&template=${state.filter.data.type}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onCopy(id) {
                        Fast.api.ajax({
                            url: `cus/decorate/template/copy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/decorate/template/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onRecyclebin() {
                        Fast.api.open(`cus/decorate/template/recyclebin`, "回收站", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDecorate(id) {
                        Fast.api.addtabs(`cus/decorate/page/index?id=${id}&from=${state.filter.data.type}`, '装修模板')
                    }

                    function onChangeStatus(item) {
                        Fast.api.ajax({
                            url: `cus/decorate/template/status/id/${item.id}`,
                            type: 'POST',
                            data: {
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/decorate/template/add' : `cus/decorate/template/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
        ...
        select: () => {
            ...
            const select = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/decorate/template/select',
                            type: 'GET',
                        }, function (ret, res) {
                            state.data = res.data
                            return false
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/decorate/template/recyclebin',
                            type: 'GET',
                            data: {
                    ...
                    function onRestore(id) {
                        Fast.api.ajax({
                            url: `cus/decorate/template/restore/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDestroy(id) {
                        Fast.api.ajax({
                            url: `cus/decorate/template/destroy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
