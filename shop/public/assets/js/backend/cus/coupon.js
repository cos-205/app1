            ...
            const index = {
                setup() {
                    ...
                    async function onCommand(item) {
                        Fast.api.ajax({
                            url: `cus/coupon/edit/id/${item.id}`,
                            type: 'POST',
                            data: {
                                status: item.type
                            },
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/coupon/add?type=add`, "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/coupon/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onRecyclebin() {
                        Fast.api.open(`cus/coupon/recyclebin`, "回收站", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onCoupon(id) {
                        Fast.api.addtabs(`cus/user/coupon?coupon_id=${id}`, '领取记录')
                    }

                    function onSend(id) {
                        Fast.api.open(`cus/user/user/select?id=${id}`, '选择用户', {
                            callback() {
                                getData()
                            }
                        })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/coupon/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                    ...
                    function onSelectCategory() {
                        ...
                        Fast.api.open(`cus/category/select?from=coupon&multiple=true`, "选择分类", {
                            callback(data) {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                ...
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/coupon/add' : `cus/coupon/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: submitForm
                                }, function (ret, res) {
                                    Fast.api.close()
