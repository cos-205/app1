            ...
            const index = {
                setup() {
                    ...
                    function onDetail(id) {
                        Fast.api.open(`cus/user/user/detail?id=${id}`, "详情", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/user/user/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
            ...
            const detail = {
                setup() {
                    ...
                    async function onSave() {
                        Fast.api.ajax({
                            url: `cus/user/user/edit/id/${state.id}`,
                            type: 'POST',
                            data: state.detail,
                        }, function (ret, res) {
                            getDetail()
                    ...
                    function onChangeParentUser() {
                        Fast.api.open(`cus/commission/agent/select?id=${state.id}`, "更换上级分销商", {
                            callback() {
                                getDetail()
                            }
                        })
                    ...
                    function onRecharge(type) {
                        Fast.api.open(`cus/user/user/recharge?type=${type}&id=${state.detail.id}`, "充值", {
                            callback: function () {
                                getDetail();
                                getLog()
                            }
                    ...
                    function getLog() {
                        ...
                        if (log.tabActive == 'money' || log.tabActive == 'score' || log.tabActive == 'commission') {
                            url = `cus/user/wallet_log/${log.tabActive}/id/${state.id}`
                        }
                        ...
                        if (log.tabActive == 'coupon') {
                            url = `cus/user/user/coupon/id/${state.id}`
                        }
                        Fast.api.ajax({
                            url: url,
                            type: 'GET',
        ...
        select: function () {
            ...
            const select = {
                setup() {
                    ...
                    function onConfirm() {
                        Fast.api.ajax({
                            url: `cus/coupon/send/id/${state.id}`,
                            type: 'POST',
                            data: {
                                user_ids: state.ids
                            },
