        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/wechat/reply/add?type=add&group=${state.group}`, "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/wechat/reply/edit?type=edit&group=${state.group}&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/wechat/reply/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onChangeStatus(item) {
                        Fast.api.ajax({
                            url: `cus/wechat/reply/edit/id/${item.id}`,
                            type: 'POST',
                            data: {
                                status: item.status,
                            },
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/wechat/reply/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            getMaterialSelect()
                    ...
                    function getMaterialSelect() {
                        Fast.api.ajax({
                            url: 'cus/wechat/material/select',
                            type: 'GET',
                            data: {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/wechat/reply/add' : `cus/wechat/reply/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
