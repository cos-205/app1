            ...
            const index = {
                setup() {
                    ...
                    function getSkuPrices(goods_id) {
                        Fast.api.ajax({
                            url: `cus/app/score_shop/skuPrices/goods_id/${goods_id}`,
                            type: 'GET',
                        }, function (ret, res) {
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/goods/goods/select?data_type=score_shop`, "选择商品", {
                            callback(data) {
                                Fast.api.open(`cus/app/score_shop/add?type=add&goods_id=${data.id}&title=${data.title}`, "添加", {
                                    callback() {
                                        getData()
                                    }
                                })
                    ...
                    function onEdit(item) {
                        Fast.api.open(`cus/app/score_shop/edit?type=edit&goods_id=${item.id}&title=${item.title}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/app/score_shop/delete/goods_id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onRecyclebin() {
                        Fast.api.open(`cus/app/score_shop/recyclebin`, "回收站", {
                            callback() {
                                getData()
                            }
                        })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getSkus() {
                        Fast.api.ajax({
                            url: `cus/app/score_shop/skus/goods_id/${state.goods_id}`,
                            type: 'GET',
                        }, function (ret, res) {
                    ...
                    function onConfirm() {
                        ...
                        Fast.api.ajax({
                            url: state.type == 'add' ? 'cus/app/score_shop/add' : `cus/app/score_shop/edit/goods_id/${state.goods_id}`,
                            type: 'POST',
                            data: submitForm,
                        }, function (ret, res) {
                            Fast.api.close()
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/app/score_shop/recyclebin',
                            type: 'GET',
                            data: {
                                page: pagination.page,
                                list_rows: pagination.list_rows,
                    ...
                    function onRestore(id) {
                        Fast.api.ajax({
                            url: `cus/app/score_shop/restore/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDestroy(id) {
                        Fast.api.ajax({
                            url: `cus/app/score_shop/destroy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
