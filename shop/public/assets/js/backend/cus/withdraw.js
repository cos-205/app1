        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onHandle(id, index, action) {
                        if (action === 'refuse') {
                            Fast.api.open(`cus/withdraw/handle/id/${id}?id=${id}`, "拒绝", {
                                callback() {
                                    getData()
                                }
                            })
                    ...
                    // 手动检查微信提现结果
                    function onCheckWechatTransferResult(id, index) {
                        handlePopover.flag[index] = false;
                        Fast.api.ajax({
                            url: `cus/withdraw/checkWechatTransferResult/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData();
                        }, function (ret, res) { })
                    ...
                    function onLog(id) {
                        Fast.api.open(`cus/withdraw/log/id/${id}?id=${id}`, "日志")
                    }
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/withdraw/handle/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
        ...
        log: () => {
            ...
            const log = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: `cus/withdraw/log/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            state.data = res.data
                            return false
