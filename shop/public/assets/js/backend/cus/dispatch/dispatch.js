        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onAdd() {
                        Fast.api.open(`cus/dispatch/dispatch/add?type=add&dispatch_type=${state.dispatch_type}`, "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/dispatch/dispatch/edit?type=edit&id=${id}&dispatch_type=${state.dispatch_type}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onCopy(id) {
                        Fast.api.open(`cus/dispatch/dispatch/add?type=copy&id=${id}&dispatch_type=${state.dispatch_type}`, "复制", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/dispatch/dispatch/delete/id/${id}?type=${state.dispatch_type}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/dispatch/dispatch/detail/id/${state.id}`,
                            type: 'GET',
                            data: {
                                type: state.dispatch_type,
                            }
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                ...
                                Fast.api.ajax({
                                    url: state.type == 'add' || state.type == 'copy' ? 'cus/dispatch/dispatch/add' : `cus/dispatch/dispatch/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: submitForm,
                                }, function (ret, res) {
                                    Fast.api.close()
