        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/pay_config/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            return false
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/pay_config/add' : `cus/pay_config/edit/id/${state.id}`,
                                    type: 'POST',
            ...
            const recyclebin = {
                setup() {
                    ...
                    function onRestore(id) {
                        Fast.api.ajax({
                            url: `cus/pay_config/restore/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDestroy(id) {
                        Fast.api.ajax({
                            url: `cus/pay_config/destroy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
