                ...
                setup() {
                    ...
                    function onBatchHandle(type) {
                        ...
                        switch (type) {
                            ...
                            default:
                                Fast.api.ajax({
                                    url: `cus/chat/common_word/edit/id/${ids.join(',')}`,
                                    type: 'POST',
                                    data: {
                                        status: type
                                    }
                    ...
                    function onAdd() {
                        Fast.api.open("cus/chat/common_word/add?type=add", "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/chat/common_word/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/chat/common_word/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/chat/common_word/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            nextTick(() => {
                    ...
                    function onConfirm() {
                        ...
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/chat/common_word/add' : `cus/chat/common_word/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
