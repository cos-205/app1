            ...
            const index = {
                setup() {
                    ...
                    function onAddStock(item) {
                        Fast.api.open(`cus/goods/stock_warning/addStock?id=${item.id}&stock=${item.stock}`, "补货", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onRecyclebin() {
                        Fast.api.open('cus/goods/stock_warning/recyclebin', "历史记录", {
                            callback() {
                                getData()
                            }
                        })
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/goods/stock_warning/addStock/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/goods/stock_warning/recyclebin',
                            type: 'GET',
                            data: {
                                page: pagination.page,
                                list_rows: pagination.list_rows,
