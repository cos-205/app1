            ...
            const index = {
                setup() {
                    ...
                    function onCommand(item) {
                        Fast.api.ajax({
                            url: `cus/goods/comment/edit/id/${item.id}`,
                            type: 'POST',
                            data: {
                                status: item.type
                            }
                    ...
                    function onAdd() {
                        Fast.api.open("cus/goods/comment/add?type=add", "添加虚拟评价", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/goods/comment/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/goods/comment/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onRecyclebin() {
                        Fast.api.open('cus/goods/comment/recyclebin', "回收站", {
                            callback() {
                                getData()
                            }
                        })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/goods/comment/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data
                            return false
                    ...
                    function onSelectFakeUser() {
                        Fast.api.open("cus/data/fake_user/select", "选择虚拟用户", {
                            callback(data) {
                    ...
                    function onChangeStatus() {
                        Fast.api.ajax({
                            url: `cus/goods/comment/edit/id/${state.id}`,
                            type: 'POST',
                            data: {
                    ...
                    function onReply() {
                        Fast.api.open(`cus/goods/comment/reply/id/${state.id}?id=${state.id}`, "回复", {
                            callback() {
                                getDetail()
                            }
                        })
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/goods/comment/recyclebin',
                            type: 'GET',
                            data: {
                                page: pagination.page,
                                list_rows: pagination.list_rows,
                    ...
                    function onRestore(id) {
                        Fast.api.ajax({
                            url: `cus/goods/comment/restore/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDestroy(id) {
                        Fast.api.ajax({
                            url: `cus/goods/comment/destroy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                ...
                setup() {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/goods/comment/reply/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
