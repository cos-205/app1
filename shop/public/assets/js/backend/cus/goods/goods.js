            ...
            const index = {
                setup() {
                    ...
                    function onCommand(item) {
                        Fast.api.ajax({
                            url: `cus/goods/goods/edit/id/${item.id}`,
                            type: 'POST',
                            data: {
                                status: item.type
                            }
                    ...
                    function onSkuCommand(item) {
                        Fast.api.ajax({
                            url: `cus/goods/sku_price/edit/id/${item.id}`,
                            type: 'POST',
                            data: {
                                status: item.type
                            }
                    ...
                    function getSkuPrice(id) {
                        Fast.api.ajax({
                            url: `cus/goods/sku_price?goods_id=${id}`,
                            type: 'GET',
                        }, function (ret, res) {
                    ...
                    function onOpenActivityDetail(activity) {
                        Fast.api.open(`cus/activity/activity/edit?type=edit&activity_type=${activity.type}&id=${activity.id}`, `${activity.type_text}活动`, {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEditStock(item) {
                        console.log('编辑库存')
                        Fast.api.open(`cus/goods/goods/addStock?id=${item.id}&stock=${item.stock || 0}&is_sku=${item.is_sku}`, "编辑库存", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onAdd() {
                        Fast.api.open("cus/goods/goods/add?type=add", "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/goods/goods/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onCopy(id) {
                        Fast.api.open(`cus/goods/goods/add?type=copy&id=${id}`, "复制", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/goods/goods/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onRecyclebin() {
                        Fast.api.open('cus/goods/goods/recyclebin', "回收站", {
                            callback() {
                                getData()
                            }
                        })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/goods/goods/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                    ...
                    function onAddCategory() {
                        Fast.api.open("cus/category/add?type=add", "添加", {
                            callback() {
                                getCategorySelect()
                            }
                        })
                    ...
                    function getDispatchSelect() {
                        Fast.api.ajax({
                            url: 'cus/dispatch/dispatch/select',
                            type: 'GET',
                            data: {
                    ...
                    function onAddDispatch(dispatch_type) {
                        Fast.api.open(`cus/dispatch/dispatch/add?type=add&dispatch_type=${dispatch_type}`, "添加", {
                            callback() {
                                getDispatchSelect()
                            }
                        })
                    ...
                    function onAddService() {
                        Fast.api.open("cus/goods/service/add?type=add", "添加", {
                            callback() {
                                getServiceSelect()
                            }
                        })
                    ...
                    function onConfirm() {
                        ...
                        setTimeout(() => {
                            if (validateData.value[0] == 0 && validateData.value[1] == 0 && validateData.value[2] == 0) {
                                ...
                                Fast.api.ajax({
                                    url: state.type == 'add' || state.type == 'copy' ? 'cus/goods/goods/add' : `cus/goods/goods/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: submitForm
                                }, function (ret, res) {
                                    Fast.api.close()
        ...
        recyclebin: () => {
            ...
            const recyclebin = {
                setup() {
                    ...
                    function getData() {
                        Fast.api.ajax({
                            url: 'cus/goods/goods/recyclebin',
                            type: 'GET',
                            data: {
                                page: pagination.page,
                                list_rows: pagination.list_rows,
                    ...
                    function onRestore(id) {
                        Fast.api.ajax({
                            url: `cus/goods/goods/restore/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
                    ...
                    function onDestroy(id) {
                        Fast.api.ajax({
                            url: `cus/goods/goods/destroy/id/${id}`,
                            type: 'POST',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
            ...
            const addStock = {
                setup() {
                    ...
                    function getSkuPrice() {
                        Fast.api.ajax({
                            url: `cus/goods/sku_price?goods_id=${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: `cus/goods/goods/addStock/id/${state.id}`,
                                    type: 'POST',
