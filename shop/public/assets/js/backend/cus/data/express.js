        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onAdd() {
                        Fast.api.open("cus/data/express/add?type=add", "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/data/express/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/data/express/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/data/express/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            return false
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/data/express/add' : `cus/data/express/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model,
                                }, function (ret, res) {
                                    Fast.api.close()
