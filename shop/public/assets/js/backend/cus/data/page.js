        ...
        index: () => {
            ...
            const index = {
                setup() {
                    ...
                    function onAdd() {
                        Fast.api.open("cus/data/page/add?type=add", "添加", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onEdit(id) {
                        Fast.api.open(`cus/data/page/edit?type=edit&id=${id}`, "编辑", {
                            callback() {
                                getData()
                            }
                        })
                    ...
                    function onDelete(id) {
                        Fast.api.ajax({
                            url: `cus/data/page/delete/id/${id}`,
                            type: 'DELETE',
                        }, function (ret, res) {
                            getData()
                        }, function (ret, res) { })
        ...
        form: () => {
            ...
            const addEdit = {
                setup() {
                    ...
                    function getDetail() {
                        Fast.api.ajax({
                            url: `cus/data/page/detail/id/${state.id}`,
                            type: 'GET',
                        }, function (ret, res) {
                            form.model = res.data;
                            return false
                    ...
                    function onConfirm() {
                        proxy.$refs['formRef'].validate((valid) => {
                            if (valid) {
                                Fast.api.ajax({
                                    url: state.type == 'add' ? 'cus/data/page/add' : `cus/data/page/edit/id/${state.id}`,
                                    type: 'POST',
                                    data: form.model
                                }, function (ret, res) {
                                    Fast.api.close()
                ...
                setup() {
                    ...
                    function onSelect(link) {
                        ...
                        if (link.path == '/pages/index/page') {
                            Fast.api.open('cus/decorate/template/select', "选择自定义页面", {
                                callback(data) {
                            ...
                            Fast.api.open('cus/category/select?from=page-category', "选择分类", {
                                callback(data) {
                                    state.selected.path += (!isEmpty(data.ids) ? '?id=' + data.ids : '');
                                }
                            })
                        } else if (link.path == '/pages/goods/list') {
                            Fast.api.open('cus/category/select?from=page-goods', "选择分类", {
                                callback(data) {
                            ...
                            Fast.api.open(`cus/activity/activity/select?type=${activityType[link.path.split('/')[3]]}`, "选择活动", {
                                callback(data) {
                                    state.selected.path += (!isEmpty(data.id) ? '?id=' + data.id : '');
                                }
                            })
                            ...
                            Fast.api.open(`cus/activity/activity/select?type=${activityType[link.path.split('/').pop()]}`, "选择活动", {
                                callback(data) {
                                    state.selected.path += (!isEmpty(data.id) ? '?activity_id=' + data.id : '');
                                    Fast.api.open(`cus/goods/goods/select?goods_ids=${data.goods_ids}`, "选择商品", {
                                        callback(data) {
                                            state.selected.path += (!isEmpty(data.id) ? '&id=' + data.id : '');
                                        }
                                    })
                            ...
                            Fast.api.open(`cus/app/score_shop/select`, "选择积分商品", {
                                callback(data) {
                                    state.selected.path += (!isEmpty(data.id) ? '?id=' + data.id : '');
                                }
                            })
                        } else if (link.path == '/pages/coupon/detail') {
                            Fast.api.open(`cus/coupon/select?status=normal`, "选择优惠券", {
                                callback(data) {
                                    state.selected.path += (!isEmpty(data.id) ? '?id=' + data.id : '');
                                }
                            })
